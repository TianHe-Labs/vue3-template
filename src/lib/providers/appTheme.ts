...
const appThemeSymbol = Symbol()

interface AppThemeCtx {
  appTheme: Ref<Nullable<GlobalTheme>>
...
export function provideAppTheme(): AppThemeCtx {
  const appTheme = useStorage<Nullable<GlobalTheme>>(
    'appTheme',
    null,
    localStorage,
  ...
  */
  ...
  return { appTheme, themeOverrides, zhCN, dateZhCN, ...handlers }
}

export function useAppTheme(): AppThemeCtx {
  return inject(appThemeSymbol) as AppThemeCtx
}
